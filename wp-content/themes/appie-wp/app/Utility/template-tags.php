...
if ( ! function_exists( 'appie_post_thumbnail' ) ) :
	
	function appie_post_thumbnail() {
...
if ( ! function_exists( 'appie_return' ) ) :

  function appie_return($arg){
...
// ----------------------------------------------------------------------------------------
if ( !function_exists('appie_get_breadcrumbs') ) {
   
	function appie_get_breadcrumbs( $seperator = '/', $word = '' ) {

		$general_breadcrumb_limit = appie_option('general_breadcrumb_limit');
		$breadcrumb_post_title    = appie_option('general_breadcrumb_post_title_show',0);
		$breadcrumb_page_title    = appie_option('general_breadcrumb_page_title_show',0);
		
		$breadcrumb_enable        = appie_option('blog_show_breadcrumb',1);
		...
		if ( is_tag() ) {
			...
			echo '<li class="breadcrumb-item">' . esc_html__( 'Author Blogs', 'appie' );
			echo '</li>';

		} elseif ( isset( $_GET[ 'paged' ] ) && !empty( $_GET[ 'paged' ] ) ) {

			echo '<li class="breadcrumb-item">' . esc_html__( 'Blogs', 'appie' );
			echo '</li>';

		} elseif ( is_search() ) {

			echo '<li class="breadcrumb-item">' . esc_html__( 'Search Result', 'appie' );
			echo'</li>';

		} elseif ( is_404() ) {

			echo '<li class="breadcrumb-item">' . esc_html__( '404 Not Found', 'appie' );
			echo'</li>';
...
/*-----------------------------
	RANDOM SINGLE TAG
------------------------------*/
if ( !function_exists( 'appie_random_tag_retrip' ) ): 
	function appie_random_tag_retrip(){
...
------------------------------*/
if ( !function_exists( 'appie_random_category_retrip' ) ): 

	function appie_random_category_retrip(){

		$blog_cat_show   = appie_option( 'blog_category', 1 );
        $single          = appie_option( 'blog_category_single',1);
        
        // post details page 
        if(is_singular('post')){

            if( !appie_option('blog_single_banner_show',1) ){
              return;
            }
...
if ( !function_exists('appie_pagination') ) {
    function appie_pagination(){
...
------------------------*/
if ( !function_exists('appie_post_navigation') ) {
    function appie_post_navigation(){
        global $post;
        ...
        <div class="single-post-navigation clearfix">
            ...
            <div class="prev-post">
                <a href="<?php echo esc_url( get_permalink($prev_post->ID) ); ?>">
                    ...
                    <div class="title-with-link">
                        <span><?php esc_html_e( 'Prev Post', 'appie' ) ?></span>
            ...
            <div class="next-post">
                <a href="<?php echo esc_url( get_permalink($next_post->ID) ); ?>">
                    <div class="title-with-link">
                        <span><?php esc_html_e( 'Next Post', 'appie' ) ?></span>
...
----------------------------------*/
if ( !function_exists('appie_comment_count_text') ) {
    function appie_comment_count_text($post_id) {
        $comments_number = get_comments_number($post_id);
        if($comments_number==0) {
            $comment_text = esc_html__('No comment', 'appie');
        }elseif($comments_number == 1) {
            $comment_text = esc_html__('One comment', 'appie');
        }elseif($comments_number > 1) {
            $comment_text = $comments_number.esc_html__(' Comments', 'appie');
        }
...
/*------------------------
    COMMENTS PAGINATION
-------------------------*/
if ( !function_exists('appie_comments_pagination') ) {
    function appie_comments_pagination(){
...
/*------------------------
    COMMENTS NAVIGATION
-------------------------*/
if ( !function_exists('appie_comments_navigation') ) {
    function appie_comments_navigation(){
        the_comments_navigation(array(
            'screen_reader_text' => ' ',
            'prev_text'          => '<i class="ti ti-angle-double-left"></i> '.esc_html__( 'Older Comments', 'appie' ),
            'next_text'          => esc_html__( 'Newer Comments', 'appie' ).' <i class="ti ti-angle-double-right"></i>',
        ));
    }
}
...
/*------------------------------
    CUSTOM COMMENT
--------------------------------*/
if ( !function_exists('appie_comment') ) {
    function appie_comment($comment, $args, $depth) {
            ...
            <div class="comment-details">
                <div class="comment-meta">
                    ...
                    <a class="comment-date" href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
                        <?php printf( esc_html__('%1s','appie'), get_comment_date() ); ?>
                    </a>
                </div>
                ...
                <div class="comment-edit">
                    <?php  edit_comment_link( esc_html__( 'Edit Comment', 'appie' ) ); ?>
                </div>
            </div>
        ...
        <li id="comment-<?php comment_ID() ?>" class="single-comment">
            <div class="comment-details">
                        ...
                        comment_reply_link( 
                            array_merge(
                                ...
                                array(
                                    ...
                                    'reply_text' => '<i class="fal fa-reply"></i>'.esc_html__( 'Reply', 'appie' ), 
                                )
                            )
                        );
                    ?>
                ...
                <em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.','appie' ); ?></em><br/>
                <?php endif; ?>
            </div>
            <?php endif; ?>
        <?php
